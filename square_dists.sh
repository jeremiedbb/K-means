...
function run {
    for i in 5 6
    do
        python run_sqd.py $n_component $n_iter $i >> results
        echo "" >> results
    done
}
...
# sklearn pairwise dist argmin no AVX2
if [ $sklpdistnoAVX2 -eq 1 ]
then
    export MKL_ENABLE_INSTRUCTIONS=AVX
    ...
    python run_sqd.py $n_component $n_iter 3 8 >> results
    echo "" >> results
    end
fi

# sklearn pairwise dist argmin AVX2
if [ $sklpdistAVX2 -eq 1 ]
then
    export MKL_ENABLE_INSTRUCTIONS=AVX2
    echo "---------- sklearn pairwise dist argmin -------------" >> results
    echo "" >> results
